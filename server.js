...
const TURN_MS = 20000; // 20-second turn limit before auto-play
...
function initGame(players, rules) {
  ...
  let first; const temp = [];
  while (true) {
    first = deck.shift();
    if (first.color !== 'wild') break;
    temp.push(first);
  }
...
// Most common color in hand — used to auto-pick a wild color
...
// ─── Turn timer (always on, 20s) ──────────────────────────────────────────────
function clearTimer(s) {
  if (s.timers?.turn) { clearTimeout(s.timers.turn); s.timers.turn = null; }
}
function startTimer(room) {
  const s = room.state;
  if (!s || s.winner) { if (s) s.turnEndsAt = null; return; }
  clearTimer(s);
  s.turnEndsAt = Date.now() + TURN_MS;
  s.timers.turn = setTimeout(() => autoPlay(room), TURN_MS);
}

// ─── Shared resolution ─────────────────────────────────────────────────────────
function endGame(room, name) {
  const s = room.state;
  s.winner = name;
  const opp = s.players.find(p => p !== name);
  s.loserHand = opp ? s.hands[opp] : [];
  s.turnEndsAt = null;
  clearTimer(s);
  recordWin(name);
  broadcast(room);
...
function applyCardEffect(room, name, card) {
  ...
  switch (card.value) {
    ...
    case 'reverse': s.players.length === 2 ? (skip = 1) : (s.direction *= -1); break;
    case 'draw2':   s.pendingDraw += 2; skip = 1; break;
    case 'wild3':   s.pendingDraw += 3; skip = 1; break;
    case 'wild4':
      ...
      break;
    case 'wild6':   s.pendingDraw += 6; skip = 1; break;
    case '7':       if (s.rules.sevenZero) doSwap(room, name); break;
    case '0':       if (s.rules.sevenZero) doSwap(room, name); break;
  }
...
// Auto-move when the 20s timer runs out
function autoPlay(room) {
  const s = room.state;
  if (!s || s.winner) return;
  const name = curPlayer(s);
  const hand = s.hands[name];

  if (s.challengePending) {
    deal(s, name, s.pendingDraw);
    s.pendingDraw = 0;
    s.challengePending = false;
  ...
  for (let i = 0; i < hand.length; i++) {
    if (canPlay(hand[i], topCard(s), s.pendingDraw, s.rules)) { idx = i; break; }
  }
  if (idx >= 0) {
...
io.on('connection', sock => {
  ...
  sock.on('playCard', ({ cardIdx, chosenColor }) => {
    ...
    if (s.draft !== null || s.winner) return;
  ...
  sock.on('drawCard', () => {
    ...
    if (!room?.state) return;
    const s = room.state;
    if (curPlayer(s) !== sock.data.playerName || s.winner) return;
  ...
  sock.on('draftDecide', ({ play, chosenColor }) => {
    ...
    if (!room?.state) return;
    const s = room.state;
    if (curPlayer(s) !== sock.data.playerName || !s.draft || s.winner) return;
  ...
  sock.on('jumpIn', ({ cardIdx }) => {
    const room = rooms[sock.data.roomId];
    if (!room?.state || !room.state.rules.jumpIn || room.state.winner) return;
    const s    = room.state;
    const hand = s.hands[sock.data.playerName];
    const card = hand[cardIdx];
    if (!card) return;
  ...
  // Quick emoji reactions between players
  sock.on('react', ({ emoji }) => {
    const room = rooms[sock.data.roomId];
    if (!room?.state) return;
